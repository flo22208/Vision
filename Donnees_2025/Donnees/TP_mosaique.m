...
Imos_non_coul = rgb2gray(Imoscoul);
[XY_mos,Res_mos] = harris(Imos_non_coul,TailleFenetre,NbPoints,k);
[XY_3,Res_3] = harris(Im3,TailleFenetre,NbPoints,k);
[XY_Cmos_2,XY_C3_2] = apparier_POI(Imosbis,XY_mos,Im3,XY_3,TailleFenetre,seuil); %%%% seulement 9 points apparayé d'après moi c'est trop peu donc imprésion sur l'équation de l'homographie, on ne peut pas baisser le seuil sinon mauvais point, il faut avoir plus de points
...
Imoscoul = mosaiquecoul(Imoscoul,Im3_coul,H2);

figure;
affichage_image(uint8(Imoscoul),'Mosaique obtenue a partir des N images couleur initiales (version 2)',1,1,1);
%SAUVEGARDE DE LA MOSAIQUE A DEUX IMAGES EN COULEUR VERSION 2
imwrite(uint8(Imoscoul),'mosaiqueN_coul.pgm');
